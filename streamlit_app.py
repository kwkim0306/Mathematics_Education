            ...
            if show_special_points:
                try:
                    ...
                    if extremum_xs:
                        ...
                        ax.scatter(x_vals, y_ext, color="red", s=2, zorder=4, label="극점", edgecolors="black")
                        if show_axis_x:
                            ax.scatter(x_vals, np.zeros_like(x_vals), color="red", s=1, zorder=4, alpha=0.8)
                            for x0 in extremum_xs:
                                ax.text(float(x0), 0, format_value(x0), color="red", fontsize=8, ha="center", va="top", fontproperties=font_prop)
                        if show_axis_y:
                            ax.scatter(np.zeros_like(y_ext), y_ext, color="red", s=1, zorder=4, alpha=0.8)
                            for y0 in y_ext:
                    ...
                    if inflection_xs:
                        ...
                        ax.scatter(x_vals, y_inf, color="green", s=2, zorder=4, label="변곡점", edgecolors="black")
                        if show_axis_x:
                            ax.scatter(x_vals, np.zeros_like(x_vals), color="green", s=1, zorder=4, alpha=0.8)
                            for x0 in inflection_xs:
                                ax.text(float(x0), 0, format_value(x0), color="green", fontsize=8, ha="center", va="top", fontproperties=font_prop)
                        if show_axis_y:
                            ax.scatter(np.zeros_like(y_inf), y_inf, color="green", s=1, zorder=4, alpha=0.8)
                            for y0 in y_inf:
